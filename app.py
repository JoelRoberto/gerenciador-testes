...
def _encontrar_adb():
    """Busca o ADB automaticamente no sistema."""
    import shutil
    # 1. Tenta via PATH do sistema
    ...
    candidatos = [
        ...
        r"C:\Users\{u}\AppData\Local\Android\Sdk\platform-tools\adb.exe".format(
            u=__import__('os').environ.get('USERNAME','')),
...
def rodar_adb(args, timeout=10):
    if not ADB:
        return "","ADB não encontrado. Instale o ADB e adicione ao PATH."
    try:
        r = subprocess.run([ADB]+args, capture_output=True, text=True,
                           timeout=timeout, encoding="utf-8", errors="replace")
        return r.stdout.strip(), r.stderr.strip()
    except FileNotFoundError:
        return "","ADB não encontrado em: "+str(ADB)
...
def main():
    ...
    if not ADB:
        st.sidebar.error("ADB não encontrado.")
        st.sidebar.caption("Instale o Android SDK ou o [scrcpy](https://github.com/Genymobile/scrcpy) e adicione ao PATH.")
        adb_ativo = False
    else:
        dispositivos = listar_dispositivos()
        if not dispositivos:
            st.sidebar.warning("Nenhum dispositivo USB encontrado.")
            st.sidebar.caption("Verifique o cabo e ative depuração USB.")
            adb_ativo = False
        else:
